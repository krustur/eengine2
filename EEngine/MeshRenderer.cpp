...
namespace EEngine
{
	...
	{
		_renderer.GetD3dImmediateContext()->IASetInputLayout(mesh->GetMaterial()->GetShader()->GetInputLayout());
		_renderer.GetD3dImmediateContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

		UINT stride = mesh->GetMaterial()->GetShader()->GetStride();
		UINT offset = 0;
		...
		mesh->GetMaterial()->GetShader()->GetEffectWorldViewProj()->SetMatrix(reinterpret_cast<float*>(worldViewProj));

		D3DX11_TECHNIQUE_DESC techniqueDescription;
		mesh->GetMaterial()->GetShader()->GetEffectTechnique()->GetDesc(&techniqueDescription);
		for (UINT p = 0; p < techniqueDescription.Passes; ++p)
		{
			mesh->GetMaterial()->GetShader()->GetEffectTechnique()->GetPassByIndex(p)->Apply(0, _renderer.GetD3dImmediateContext());
